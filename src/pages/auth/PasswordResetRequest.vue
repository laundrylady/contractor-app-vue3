    ...
    <q-layout view="lHh Lpr lFf">
      ...
      <q-page class="row justify-center items-center animated fadeIn" padding>
        <q-card class="col-xs-12 col-sm-6 col-md-4 text-center">
          ...
          <q-card-section>
            <div class="text-h3 text-bold">Password Reset</div>
            <div class="text-h6 sub">Enter your registered email address to start the process.</div>
          </q-card-section>
          <q-card-section>
            <transition enter-active-class="animated bounceIn" leave-active-class="animated bounceOutTop" appear>
              <q-banner class="text-positive q-mb-md" v-if="success||error">
            ...
            <q-btn @click="send()" :disabled="loading||$v.$invalid" :loading="loading" color="secondary"  label="Send"
                   aria-label="Send reset password email" class="full-width"/>
            <div class="text-left q-mt-lg">
              <router-link :to="{name:'signIn'}" class="link" aria-label="Go back to Sign In">Back to Sign In</router-link>
            </div>
          </q-card-section>
        </q-card>
      </q-page>
