import { g as computed, B as defineComponent, r as ref, S as unref, m as openBlock, K as createBlock, L as withCtx, l as createVNode, O as QIcon, v as normalizeProps, aH as guardReactiveProps, aj as createTextVNode, aI as toDisplayString, z as createCommentVNode, o as onMounted, w as watch, aJ as QSpinner } from "./index.7ec40092.js";
...
import { u as useCommonStore } from "./common.e614d236.js";
    ...
    const filterPostcodes = (val, update) => {
      ...
      api.get(`/public/postcoderegion/index?keyword=${val}${props.state ? `&state=${props.state}` : ""}`).then((response) => {
        update(() => {
          postcodes.value = response.data.map((o) => {
            var _a;
            return { value: o.id, label: `${o.locality} (${((_a = common.value) == null ? void 0 : _a.operating_country) === "aud" ? `${o.state} ` : ""}${o.postcode})` };
          });
          loading.value = false;
        });
      }).catch((error) => {
      ...
      if (props.modelValue && (Array.isArray(props.modelValue) && props.modelValue.length || !Array.isArray(props.modelValue))) {
        api.get(`/public/postcoderegion/${props.modelValue}`).then((res) => {
          ...
          if (Array.isArray(res.data)) {
            ...
            postcodes.value = [{ value: res.data.id, label: `${res.data.locality} (${((_a = common.value) == null ? void 0 : _a.operating_country) === "aud" ? `${res.data.state} ` : ""}${res.data.postcode})` }];
            loading.value = false;
          }
        });
      }
