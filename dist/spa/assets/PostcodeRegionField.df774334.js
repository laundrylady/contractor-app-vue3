import { g as computed, B as defineComponent, r as ref, S as unref, m as openBlock, K as createBlock, L as withCtx, l as createVNode, O as QIcon, v as normalizeProps, aI as guardReactiveProps, ak as createTextVNode, aj as toDisplayString, z as createCommentVNode, o as onMounted, w as watch, aJ as QSpinner } from "./index.f7c5cca4.js";
...
import { u as useCommonStore } from "./common.dda69453.js";
    ...
    const filterPostcodes = (val, update) => {
      ...
      api.get(`/public/postcoderegion/index?keyword=${val}${props.state ? `&state=${props.state}` : ""}`).then((response) => {
        update(() => {
          postcodes.value = response.data.map((o) => {
            var _a;
            return { value: o.id, label: `${o.locality} (${((_a = common.value) == null ? void 0 : _a.operating_country) === "aud" ? o.state : o.region})` };
          });
          loading.value = false;
        });
      }).catch((error) => {
      ...
      if (props.modelValue && (Array.isArray(props.modelValue) && props.modelValue.length || !Array.isArray(props.modelValue))) {
        api.get(`/public/postcoderegion/${props.modelValue}`).then((res) => {
          ...
          if (Array.isArray(res.data)) {
            postcodes.value = res.data.map((o) => {
              var _a2;
              return { value: o.id, label: `${o.locality} (${((_a2 = common.value) == null ? void 0 : _a2.operating_country) === "aud" ? o.state : o.region})` };
            });
            loading.value = false;
          } else {
            postcodes.value = [{ value: res.data.id, label: `${res.data.locality} (${((_a = common.value) == null ? void 0 : _a.operating_country) === "aud" ? res.data.state : res.data.region})` }];
            loading.value = false;
          }
        });
      }
...
export { _sfc_main$2 as _, _sfc_main as a, _sfc_main$1 as b, useMixinCommon as u };
